...
function IzinForm() {
  ...
  useEffect(()=>{
    const getLeaveReasons = async() => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/getLeaveReasons`);
  ...
  return (
    <form className="text-black p-4" onSubmit={handleSubmit}>
      <div className="grid grid-cols-2 gap-2 gap-x-4">
        ...
        <div>
          <label className="block mb-2 font-semibold">İzin Başlangıç Tarihi:</label>
          <input
            type="datetime-local"
        ...
        <div className="relative">
          <label onClick={openReasonSelect} className="block mb-2 font-semibold underline text-red-600">
            İzin Nedeni Seçmek İçin Tıklayınız
          </label>
          {selectedReason && <span className="font-bold">Seçili neden: {selectedReason}</span>}
          {isOpenReason && (
            <div className="w-auto h-auto absolute z-50 bg-blue-500">
              <div className="flex flex-col h-[300px] overflow-auto rounded-md shadow-md bg-slate-400 font-semibold">
                {leaveResons && leaveResons.map((item, index) => (
                  <span
        ...
        <div>
          <label className="block mb-2 font-semibold">Ulaşılabilecek Tel:</label>
          <input
            type="tel"
            name="tel"
            className="w-full p-2 border rounded-md"
        ...
        <div>
          <label className="block mb-2 font-semibold">Ulaşılabilecek Adres:</label>
          <textarea
            name="adres"
            className="w-full p-2 border rounded-md"
            rows="3"
