...
function FinishedWorkPopup() {
  ...
  const finishedWork = async () => {
    try {
      if (
        selectedOrder &&
        selectedOrder.work_status === "1" || selectedOrder.work_status ==="2" &&
        finishedAmount !== ""
      ) {
        const response = await axios.post(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/finishedWork`,
          {
            ...
            repair_reason:selectedRepairReason?.repair_reason,
            scrap_reason:selectedScrapReason?.scrap_reason
          }
        );
                      ...
                      {repairReasonsList &&
                        repairReasonsList.map((item, index) => (
                          <li
                            ...
                            onClick={() => handleSelectedRepairReason(item)}
                            className={`${
                              item?.repair_reason_id ===
                              selectedRepairReason?.repair_reason_id
